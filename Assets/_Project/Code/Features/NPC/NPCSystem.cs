...
namespace _Project.Code.Features.NPC
{
    public class NPCSystem : MonoBehaviour, ICharacterSystem
    {
        ...
        public float HealthPercent => currentHealth / _config.health;
        ...
        public bool TryInitialize(_Project.Code.Features.Character.MB.Character character, CharacterSystemConfig cfg)
        {
            ...
            Debug.Log($"NPC инициализирован: Тип={npcType}, Здоровье={currentHealth}");
            return true;
        }
        ...
        private void UpdateState()
        {
            ...
            {
                _alertTimer -= UnityEngine.Time.deltaTime;
                if (_alertTimer <= 0)
                {
                    ChangeState(NPCState.Idle);
                }
        ...
        private void IdleBehavior()
        {
            if (agent != null && agent.remainingDistance < 0.5f)
            {
                ...
                {
                    Vector3 randomPoint = _startPosition + UnityEngine.Random.insideUnitSphere * 5f;
            ...
            if (animator != null)
                animator.SetFloat("Speed", agent?.velocity.magnitude ?? 0);
        }
        
        private void AlertBehavior()
        {
            if (_player != null && agent != null)
            {
                Vector3 direction = (_player.position - transform.position).normalized;
                direction.y = 0;
                transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction), UnityEngine.Time.deltaTime * 5f);
            }
        ...
        private void AttackBehavior()
        {
            ...
            if (distance < _config.attackRange)
            {
                ...
                {
                    _nextAttackTime = UnityEngine.Time.time + 1f;
                    PerformAttack();
                }
        ...
        private void PerformAttack()
        {
            ...
            {
                case MutationEffect.Poisonous:
                    finalDamage *= 0.8f;
                    break;
                case MutationEffect.Gigantic:
                    finalDamage *= 1.5f;
                    break;
            }
        ...
        public void ChangeState(NPCState newState)
        {
            ...
            Debug.Log($"NPC {name} сменил состояние: {oldState} → {newState}");
            
            if (agent != null && newState == NPCState.Idle)
            {
